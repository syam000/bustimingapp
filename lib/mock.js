...
function buildNextBus(rand, minutesFromNow, originCode, destCode, now) {
  const eta = new Date(now + minutesFromNow * 60_000);
  return {
...
export function getMockArrivals(busStopCode) {
  ...
  // Derive one timestamp for the whole tick and reuse it for every
  // buildNextBus() call below, so all ETAs generated within the same tick
  // are consistent (and getMockArrivals stays deterministic for tests).
  const now = tick * 20_000;
  ...
  const Services = serviceNos.map((serviceNo, idx) => {
    ...
    return {
      ServiceNo: serviceNo,
      Operator: operator,
      NextBus: buildNextBus(rand, base, originCode, destCode, now),
      NextBus2: buildNextBus(rand, base + 8 + Math.floor(rand() * 6), originCode, destCode, now),
      NextBus3: buildNextBus(rand, base + 18 + Math.floor(rand() * 8), originCode, destCode, now),
    };
  });
